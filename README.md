...
**Yêu cầu:** Python 3.11 trở lên. Cần **OpenRouter hoặc OpenAI API key** để chạy
`domain_assistant.py` (Part 3 — sinh 20 actual answers từ RAG thật); phần code
core (`template.py`, Part 1–2) không cần API key. OpenRouter có thể dùng model
`openrouter/free` cho bài lab.
...
cp .env.example .env                     # rồi điền OPENROUTER_API_KEY hoặc OPENAI_API_KEY
```
