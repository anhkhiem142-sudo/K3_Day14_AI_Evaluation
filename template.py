...
class LLMJudge:
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        positional_bias = (
            position_comparisons > 0
            and first_wins / position_comparisons > 0.8
        )
        return {
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures):
            suggestion = (
                ...
                else "Review the failure and update the evaluation pipeline"
            )
            values = (
                f"F{index + 1:03d}",
    ...
    def generate_improvement_suggestions(
        ...
        suggestions_by_type = {
            "hallucination": (
                "Add grounding checks and require citations for unsupported claims"
            ),
            "irrelevant": (
                "Clarify the answer prompt and add intent-specific routing examples"
            ),
            "incomplete": (
                "Improve retrieval coverage and add a checklist of required answer points"
            ),
            "off_topic": (
                "Strengthen intent detection and reject context from unrelated domains"
            ),
            "refusal": (
                "Recalibrate guardrails with valid examples that should be answered"
            ),
        }
        ...
        suggestions = [
            suggestions_by_type.get(
                failure_type.lower(),
                f"Review {failure_type} failures and add representative regression tests",
            )
            for failure_type in prioritized_types
        ]

        fallbacks = [
            "Add representative failed cases to the golden regression dataset",
            "Track answer and retrieval metrics separately to isolate root causes",
